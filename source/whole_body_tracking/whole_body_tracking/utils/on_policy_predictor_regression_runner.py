"""
On-policy runner variant with an auxiliary regression predictor for racket command.

- Maintains a ping-pong ball trajectory buffer per env from `TTEnv.ball_pos`.
- Trains a small MLP to predict a future racket command from the last H positions.
- Performs inference every step and, if available, calls `env.update_racket_command`.
"""
...
class _MLPPredictor(torch.nn.Module):
    def __init__(self, input_dim: int, hidden_sizes: Tuple[int, int] = (64, 64), output_dim: int = 7):
        super().__init__()
        h1, h2 = hidden_sizes
...
class OnPolicyPredictorRegressionRunner(OnPolicyRunner):  # noqa: C901
    """Runner with auxiliary regression predictor for future racket command."""
    ...
    def __init__(self, env, train_cfg: dict, log_dir: str | None = None, device: str = "cpu"):
        ...
        # Ground-truth future racket command buffer on CPU: [T, N, 7]
        self._gt_buf_cpu: torch.Tensor = torch.zeros(
            self._traj_maxlen, self.env.num_envs, 7, dtype=torch.float32, device="cpu"
        )
        ...
        self._predictor = _MLPPredictor(self._pred_input_dim, self.pred_hidden, 7).to(self.device)
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        for it in range(start_iter, tot_iter):
            ...
            # ==========================================
            # 🌟 新增：计算训练进度并传递给环境 (0.0 -> 1.0)
            # ==========================================
            progress = (it - start_iter) / max(1, num_learning_iterations)
            base_env = getattr(self.env, "unwrapped", self.env)
            base_env.episode_progress_ratio = progress
    ...
    # ---------------------------
    def _record_ball_positions_legacy(self):
        base_env = getattr(self.env, "unwrapped", self.env)
        if not hasattr(base_env, "ball_pos"):
            return
        
        with torch.no_grad():
            ball_pos = base_env.ball_pos.detach().to("cpu")
            # 🌟 先写入数据
            self._traj_buf_cpu[self._traj_write_idx].copy_(ball_pos)
            
            try:
                if self.current_learning_iteration < self.pred_train_until_iters:
                    if hasattr(base_env, "ball_future_pose"):
                        gt_pose = base_env.ball_future_pose.detach().to("cpu")
                        # 🌟 写入对应的真值
    ...
    def _maybe_predict_and_update_env(self):
        # print(f"RUN PREDICT")
        """Run predictor inference given last H positions and update env with racket command."""
        ...
        base_env = getattr(self.env, "unwrapped", self.env)
        try:
            if hasattr(base_env, "update_racket_command"):
                base_env.update_racket_command(preds)
            elif hasattr(base_env, "update_prediction"):
                base_env.update_prediction(preds[:, 0:3])
    ...
    def _train_predictor_offline(self) -> Optional[float]:
        ...
        Targets are taken directly from the environment-provided racket command
        ...
        gt_seq = self._gt_buf_cpu[idxs]  # [L, N, 7] on CPU
        ...
        try:
            if getattr(base_env, "add_noise", False):
    ...
    # ---------------------------
    # Override: record GT racket commands
    # ---------------------------
    def _record_ball_positions(self):
        ...
        with torch.no_grad():
            ...
            try:
                if self.current_learning_iteration < self.pred_train_until_iters:
                    if hasattr(base_env, "racket_cmd_gt"):
                        gt_cmd = base_env.racket_cmd_gt.detach().to("cpu")
                        self._gt_buf_cpu[self._traj_write_idx].copy_(gt_cmd)
                    elif (
                        hasattr(base_env, "ball_future_pose")
                        and hasattr(base_env, "ball_linvel")
                        and hasattr(base_env, "ball_future_t")
                    ):
                        gt_pose = base_env.ball_future_pose.detach().to("cpu")
                        gt_vel = base_env.ball_linvel.detach().to("cpu")
                        gt_t = base_env.ball_future_t.detach().to("cpu").unsqueeze(-1)
                        gt_cmd = torch.cat([gt_pose, gt_vel, gt_t], dim=-1)
                        self._gt_buf_cpu[self._traj_write_idx].copy_(gt_cmd)
            except Exception:
                pass
