...
def get_ball_prediction(env: "TableTennisEnv"):
    """Legacy 3D predictor output for debug only."""
    return env.ball_prediction

def get_ball_hit_prediction(env: "TableTennisEnv"):
    """Structured future hit-state prediction [p_hit(3), v_ball_hit(3), t_hit(1)]."""
    return env.ball_hit_prediction

# def get_rel_target_xy(env: "TableTennisEnv"):
#     """Legacy predictor-based robot shift for debug only."""
...
def get_base_target_xy(env: "TableTennisEnv"):
    """Per HITTER Table I: p̂_base,xy − p_base,xy (delta in table frame)."""
    base_xy = (env.scene["robot"].data.root_pos_w - env.scene["table"].data.root_pos_w)[:, :2]
    return env.base_target_xy - base_xy

def get_racket_target_pos(env: "TableTennisEnv"):
    """Per HITTER Sec V-B1: p̂_racket relative to base (world-axis-aligned)."""
    base_w = env.scene["robot"].data.root_pos_w
    table_w = env.scene["table"].data.root_pos_w
    racket_target_table = env.racket_cmd[:, 0:3]
    base_table = base_w - table_w
    return racket_target_table - base_table

def get_racket_target_vel(env: "TableTennisEnv"):
    """Per HITTER Sec V-B1: v̂_racket in world frame."""
    return env.racket_cmd[:, 7:10]
...
def get_reference_joint_pos_vel(env: "TableTennisEnv"):
    ...
    try:
        ...
        q_bh = cmd_bh.joint_pos
        qd_bh = cmd_bh.joint_vel
        q = torch.where(env.swing_type[:, None] == 0, q_fh, q_bh)
        qd = torch.where(env.swing_type[:, None] == 0, qd_fh, qd_bh)
        return torch.cat([q, qd], dim=-1)
    except Exception:
        num_joints = env.scene["robot"].num_joints
        zeros = torch.zeros((env.scene.num_envs, num_joints), device=env.device, dtype=torch.float32)
        return torch.cat([zeros, zeros], dim=-1)

def get_serve_progress(env: "TableTennisEnv"):
    """Serve progress (t_serve / t_serve,max)"""
    return (env.ball_episode_length_buf.float() / 150.0).unsqueeze(-1)
