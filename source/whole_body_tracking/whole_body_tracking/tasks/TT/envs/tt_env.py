...
from whole_body_tracking.tasks.TT.mdp.command_sampler import TTCommandSampler, TTCommandSamplerCfg
...
class TableTennisEnv(ManagerBasedRLEnv):
    ...
    def __init__(self, cfg: TableTennisEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # 相位与击球决策
        self.episode_swing_type = torch.zeros(num_envs, device=device, dtype=torch.long)
        ...
        # 直接绑定 URDF 中已存在的 racket_link（fixed joint to right_wrist_yaw_link，
        # transform xyz="0.12 0 0" rpy="0 1.5 -1.5"）。读 body pose 与物理碰撞对齐，
        # 不再手算 wrist+offset，避免一致性 bug。
        paddle_body_name = "racket_link"
        found_ids, _ = self.robot.find_bodies([paddle_body_name], preserve_order=True)
        if len(found_ids) == 0:
            # 后备：URDF 缺 racket_link 时回退到 wrist + 解析 offset
            print("[WARNING] racket_link not found in articulation; falling back to wrist+offset.")
            paddle_body_name = "right_wrist_yaw_link"
            found_ids, _ = self.robot.find_bodies([paddle_body_name], preserve_order=True)
            if len(found_ids) == 0:
                raise RuntimeError("Failed to find any paddle carrier body.")
            self.paddle_index = found_ids[0]
            self._paddle_use_direct = False
            ...
            self.paddle_index = found_ids[0]
            self._paddle_use_direct = True

        # Phase 1：解耦训练采样器。替代旧的 planner_*/predictor 混合逻辑。
        self.command_sampler = TTCommandSampler(self, TTCommandSamplerCfg())
    ...
    def _select_swing_type_from_hit_state(self, hit_pos: torch.Tensor) -> torch.Tensor:
        ...
        planned = torch.where(choose_forehand, torch.zeros_like(planned), planned)
        
        ambiguous = torch.abs(forehand_cost - backhand_cost) <= (2.0 * margin)
        planned = torch.where(ambiguous, self.episode_swing_type, planned)

        random_prob = float(getattr(self.cfg, "swing_random_prob", 0.0))
        if random_prob > 0.0:
            random_pick = torch.randint(0, 2, planned.shape, device=self.device, dtype=planned.dtype)
            random_mask = (torch.rand(self.num_envs, device=self.device) < random_prob) & comfortable
            planned = torch.where(random_mask, random_pick, planned)
        return planned

    def _plan_base_target(self, hit_pos: torch.Tensor) -> torch.Tensor:
        forehand = (self.swing_type == 0)
        ...
        y_offset = torch.where(forehand, torch.full_like(hit_pos[:, 1], -0.22), torch.full_like(hit_pos[:, 1], 0.22))
    ...
    def _plan_racket_command(self, hit_state: torch.Tensor) -> torch.Tensor:
        ...
        forehand = (self.swing_type == 0).unsqueeze(-1)
        lateral_offset = torch.where(forehand, hit_pos.new_tensor([0.00, -0.08, 0.02]), hit_pos.new_tensor([0.00, 0.08, 0.02]))
    ...
    def update_planned_commands(self):
        ...
        lock_t_min = float(getattr(self.cfg, "swing_lock_t_min", 0.05))
        lock_t_max = float(getattr(self.cfg, "swing_lock_t_max", 0.75))
        
        # 动态判定当前的接球倾向并上锁
        choose_now = (~self.swing_type_locked) & (hit_state[:, 6] > lock_t_min) & (hit_state[:, 6] < lock_t_max) & (~self.mask_invalid)
        if torch.any(choose_now):
            self.swing_type[choose_now] = self._select_swing_type_from_hit_state(hit_state[:, :3])[choose_now]
            self.swing_type_locked[choose_now] = True
        ...
        self.base_target_xy[:] = torch.where(
            ...
            torch.lerp(self.base_target_xy, default_target_xy, cmd_alpha)
        )
    ...
    def _update_motion_phase_from_planner(self):
        ...
        # 🌟 核心修复：允许在球出界或打完后解除锁定状态，防止机器人僵死在空中！
        release_now = self.has_touch_paddle | (~valid_ball)
        if torch.any(release_now):
            self.swing_type_locked[release_now] = False
            # 重新给无效的球随机一个默认偏好，保证下一轮规划能顺滑过渡
            self.episode_swing_type[release_now] = torch.randint(
                0, 2, (int(release_now.sum().item()),), device=self.device, dtype=torch.long
            )
        ...
        try:
            ...
            self.command_manager.get_term("motion_backhand").set_motion_phase(self.motion_phase_time.clone())
        except Exception: pass

    def _compute_racket_pose_from_wrist(self):
        # 当 paddle_index 指向 racket_link 时，URDF 已经把 wrist→racket 的固定 transform
        # 烘进刚体姿态，直接读 body_*_w 即可（与物理碰撞同源，无需再叠 offset）。
        pos = self.robot.data.body_pos_w[:, self.paddle_index, :]
        quat = self.robot.data.body_quat_w[:, self.paddle_index, :]
        linvel = self.robot.data.body_lin_vel_w[:, self.paddle_index, :]
        if getattr(self, "_paddle_use_direct", False):
            return pos, quat, linvel

        # Fallback: paddle_index 指向 wrist，叠加解析 offset 还原 racket 中心
        angvel = self.robot.data.body_ang_vel_w[:, self.paddle_index, :]
        offset_pos_w = math_utils.quat_apply(quat, self.racket_offset_pos.unsqueeze(0).expand(self.num_envs, -1))
        return (
            pos + offset_pos_w,
            math_utils.quat_mul(quat, self.racket_offset_quat),
            linvel + torch.cross(angvel, offset_pos_w, dim=1),
        )
    ...
    def compute_perception(self):
        ...
        # self.robot_pos = self.robot.data.root_pos_w - self.scene.env_origins
    ...
    def _reset_idx(self, env_ids: torch.Tensor):
        ...
        self.episode_swing_type[env_ids] = torch.randint(0, 2, (len(env_ids),), device=self.device, dtype=torch.long)
        self.motion_phase_time[env_ids] = 0.0
        self.motion_phase_target_time[env_ids] = 0.0

        # Phase 1：初始命令由 sampler 生成（写 racket_cmd / base_target_xy /
        # t_strike / swing_type / motion_phase_time）。
        self.command_sampler.reset(env_ids)
    ...
    def _reset_ball_async(self, env_ids):
        ...
        # 球物理状态 flags（在 Phase 1 训练中不参与 reward，仅供 evaluation 读取）
        ...
        # Phase 1：racket_cmd / base_target_xy / t_strike / swing_type 由
        # TTCommandSampler 唯一持有，这里不再覆写。
