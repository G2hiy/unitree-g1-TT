...
from whole_body_tracking.tasks.bydmimic.mdp.commands import MotionCommand
from whole_body_tracking.tasks.bydmimic.mdp.rewards import _get_body_indexes
...
def base_height_below_minimum(
    env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"), threshold: float = 0.38
) -> torch.Tensor:
    asset: RigidObject | Articulation = env.scene[asset_cfg.name]
...
def bad_upright_orientation(
    env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"), threshold: float = 0.85
) -> torch.Tensor:
    asset: RigidObject | Articulation = env.scene[asset_cfg.name]
