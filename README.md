...
- source/whole_body_tracking/whole_body_tracking/tasks/tracking/mdp 此目录包含定义 BeyondMimic 项目 MDP（马尔可夫决策过程）的原子函数。下面是这些函数的具体功能分解：

 - commands.py 命令库，用于计算来自参考运动、当前机器人状态和误差计算的相关变量。这包括姿态和速度误差计算、初始状态随机化以及自适应采样。

 - rewards.py 实现 DeepMimic 奖励函数和平滑项。

 - events.py 实现领域随机化（Domain Randomization）项。

 - observations.py 实现用于运动跟踪和数据收集的观测项。

 - terminations.py 实现早期终止和超时机制。

- source/whole_body_tracking/whole_body_tracking/tasks/tracking/tracking_env_cfg.py 包含跟踪任务的环境（MDP）超参数配置(奖励函数的权重也在这设置)。

- source/whole_body_tracking/whole_body_tracking/tasks/tracking/config/g1/agents/rsl_rl_ppo_cfg.py 包含跟踪任务的 PPO（近端策略优化）超参数。

- source/whole_body_tracking/whole_body_tracking/robots 包含机器人特定的设置，包括电枢参数（armature parameters）、关节刚度/阻尼计算，以及动作缩放计算。

- scripts 包括用于预处理运动数据、训练策略和评估已训练策略的实用脚本。
